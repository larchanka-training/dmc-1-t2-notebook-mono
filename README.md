...
В проекте используются git submodules для frontend и backend.

При первом клонировании используйте:

```
git clone --recurse-submodules <repo-url>
```

Если репозиторий уже склонирован, но папки `ui` и `api` пустые, выполните в корне проекта:

```
git submodule update --init --recursive
```

После этого в папках `ui` и `api` появится код соответствующих сервисов.

## Обновление репозитория

После обновления монорепозитория выполните:
...
`git pull` обновляет указатели сабмодулей, зафиксированные в монорепозитории.
Команда `git submodule update --init --recursive` приводит содержимое папок `ui` и `api` к этим commit-ам.
