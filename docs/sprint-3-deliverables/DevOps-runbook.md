...
- **Notebooks live in offline-first IndexedDB** on the client: users'
  **local** notebooks **are not lost** even if the DB is completely
  gone — only the server-synchronized copies. Background autosync
  (see `ui` remoteSync, `larchanka-training/js-notebook#134`) pushes
  edits to the server while signed in, so server-side state is
  usually close to user state; nevertheless the **user-side store
  survives any backend outage**. **Browser execution (QuickJS) and
  in-browser AI (WebLLM) continue to work while the backend is fully
  down.** This **reduces the user impact** of Scenario A from a full
  outage to "cannot sign in + new edits don't autosync until backend
  is back".
...
- ✅ **CloudWatch metric filter `jsnotes-t2-ai_request`** — counts
  ...
  `/ecs/jsnotes-t2-api`), namespace `JSNotebook/Events`,
...
- ❌ AWS Budget alert is **absent** (`deploy-user` lacks
  `budgets:ModifyBudget`);
- ❌ Cost Explorer lag remains **24–48 hours**.

So today an overrun is detected **only if someone looks at the
dashboard** — the realtime signal exists, but is not pushed.
...
2. **Add a per-token metric filter** (optional, catches long-response
   bursts that pure request-count would miss):

   ```hcl
   resource "aws_cloudwatch_log_metric_filter" "llm_total_tokens" {
     name           = "${var.project}-llm-total-tokens"
     log_group_name = aws_cloudwatch_log_group.api.name
     pattern        = "{ $.event = \"ai_request\" }"
     metric_transformation {
       name      = "AIRequestTokens"
       namespace = "JSNotebook/Events"
       value     = "$.total_tokens"
       unit      = "Count"
     }
...
aws logs start-query --log-group-name /ecs/jsnotes-t2-api \
  ...
  --query-string 'filter event="ai_request" | stats count() by user_id | sort count desc | limit 20'
# Save the queryId, then call get-query-results
```
...
- **`LLM_CLOUD_AGENT_ENABLED` flag (HIGH PRIORITY)** — confirmed
  ...
  `api/app/` and absent from the active TD (revision 44 on that
  date). §9.4.2 Level 1 in this runbook describes the target/future
  state. **In a real incident today — only Level 2 (IAM revoke)
  works.** Tracked: `larchanka-training/dmc-1-t2-notebook-api#74`
  (api + a sibling Terraform PR in `mono`).
...
- **CloudWatch alarm on Bedrock daily cost** — via the Budgets API
  (after extending `deploy_user` rights). Until then a manual Console
  budget is the answer (§9.5).
...
```text
filter event = "ai_request" and duration_ms > 5000
