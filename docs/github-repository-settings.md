...
| Infra — Provision prod host | `workflow_dispatch` | Не required | Разовый bootstrap прод-EC2 |
...
`Deploy` workflow делает **реальный SSH-выкат** на постоянный прод-хост (создан
один раз через `infra-prod.yml`): ECR login → `docker compose pull && up -d` →
smoke `curl /api/v1/health`. Запускается автоматически после `ECR Publish` на
`main` (`workflow_run`) и вручную (`workflow_dispatch`) для отката. Если SSH-секреты
не заданы — остаётся dry-run. Деталь — [`deploy.md`](deploy.md).
...
| Bootstrap прод-хоста (EC2 + SG + Docker) | `.github/workflows/infra-prod.yml` |
