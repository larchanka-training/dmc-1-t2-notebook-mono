...
| Preview Environment | PR (`api`/`ui`/`proxy`/compose) | Не required | Собирает `pr-<N>` образы + коммент в PR; деплой — scaffold |
| Deploy | авто после `ECR Publish` на `main` (`workflow_run`) + `workflow_dispatch` | Не required | Не PR gate; dry-run выкат / откат |
...
Текущий `Deploy` workflow — dry-run: валидирует `image_tag` и
`docker-compose.prod.yaml`, но не подключается к серверу. Запускается
автоматически после `ECR Publish` на `main` (`workflow_run`) и вручную
(`workflow_dispatch`) для отката. Реальные secrets для AWS/SSH/deploy
добавляются после выбора инфраструктуры (Terraform).
...
| `SSH_HOST` / `SSH_USER` / `SSH_PRIVATE_KEY` | реальный deploy (позже) | SSH на сервер; `deploy.yml` проверяет их наличие |
| `DATABASE_URL`, `OAUTH_*`, `*_TTL` | API deploy позже | Прод-конфиг БД/OAuth/TTL (в воркфлоу пока не используются) |
...
| Deploy (auto + manual) dry-run | `.github/workflows/deploy.yml` |
