...
- **Prompt-injection pre-filter (backend).**
  ...
  **Threat-model shift to call out explicitly.** The guard is no longer a barrier for prompt-injection embedded **inside** context cells: the generator receives the full, untruncated context, and any "ignore previous instructions"-style content there reaches the generator unchanged. Two notes on why this is the accepted trade-off in the MVP:
    ...
    - In a **shared / multi-user notebook** (a future scenario), a collaborator's malicious markdown cell *could* try to steer the generator. That generator surface is **not new** — it was equally exposed before this change; the previous behaviour merely incidentally short-circuited the call with a blunt 422. The shared-notebook injection surface is owned by a separate hardening task — switching to a rule-based pre-check or **AWS Bedrock Guardrails** (out of scope here) — not by this LLM-prompted classifier, since "instruct the model to ignore instructions in the data" is not a robust defence.
